 ...
 */
public class EditFragment extends Fragment implements ViewOpenEditPop, SensorEventListener {
    ...
    public void onResume() {
        super.onResume();
        mSensorManager.registerListener(EditFragment.this, mSensor, SensorManager.SENSOR_DELAY_NORMAL);
    }

    @Override
    public void onPause() {
        super.onPause();
        mSensorManager.unregisterListener(EditFragment.this);
    }
    ...
    public void showEditPopWindow(int type, View widget, int layoutID) {
        ...
        if (type == Contacts.ROCKERVIEW_TYPE) {
            ...
            RockerPopAction rockerPopAction = new RockerPopAction((RockerView) widget, parentView, getActivity());

        } else if (type == Contacts.WHELLVIEW_TYPE) {

            WheelPopAction wheelPopAction = new WheelPopAction((WheelView) widget, parentView, getActivity());

        } else if (type == Contacts.SEEKBARVIEW_TYPE) {

            SeekBarPopAction seekBarPopAction = new SeekBarPopAction((SeekBarView) widget, parentView, getActivity());

        } else if (type == Contacts.CIRCLEBUTTON_TYPE) {

            CircleButtonPopAction action = new CircleButtonPopAction((CircleButton) widget, parentView, getActivity());

        } else if (type == Contacts.SWTCHVIEW_TYPE) {

            SwitchPopAction switchPopAction = new SwitchPopAction((SwitchView) widget, parentView, getActivity());

        } else if (type == Contacts.THREEVIEW_TYPE) {
            ThreePopAction threePopAction = new ThreePopAction((ThreeView) widget, parentView, getActivity());


        } else if (type == Contacts.KNOBTABLE_TYPE) {

            KnobPopAction knobPopAction = new KnobPopAction((KnobTableView) widget, parentView, getActivity());
        } else if (type == Contacts.OUTPUTVIEW_TYPE) {

            OutputPopAction outputPopAction = new OutputPopAction((OutputView) widget, parentView, getActivity());
        }
    ...
    public ThreeView createThreeView() {
        ThreeView threeView = new ThreeView(getActivity(), this);
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        bundle.putInt("y", y);
        bundle.putInt("z", z);
        ThreeView threeView = new ThreeView(getActivity(),bundle);

        Log.i("kevin", "x" + x + "y" + y + "z" + z);
